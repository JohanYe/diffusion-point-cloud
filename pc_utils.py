...
class ChamferLoss(nn.Module):
    ...
    def forward(self, ground_truth, reconstruction, surface_normals_tuple=None):
        ...
        if isinstance(surface_normals_tuple, tuple):
            ...
            return {"total_loss": (loss_1 + loss_2 + beta * angle.mean()) * 1000,
                    "chamfer": (loss_1 + loss_2) * 1000,
                    "surface_normal_angle": angle.mean() * 1000,
...
def save_ply_manual(filename, verts, faces):
...
def facets_from_grid(num_points):
