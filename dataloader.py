...
def all_unn(data_dir):
    ...
    if platform.system() == "Linux":
        xml_file = [file for file in xml_file if "linux" in file]
    else:
        xml_file = [file for file in xml_file if "linux" not in file]
    ...
    for e in root.iter("DataItem"):
        ...
        if int(data_element.text) < 33:  # exclude a small amount of teeth with labels over 32 (4k out of 130k)
            data_info.append(d)
    return data_info


class Teeth_Dataset(Dataset):
    def __init__(self, unn, folder_path, is_train, args, global_pc_std=None, cluster=False):
        self.is_train = is_train
        self.data_folder_path = folder_path
        self.cluster = cluster
        self.k = 2048 if is_train else 2048
        self.point_normals = args.point_normals
        self.all_points_mean = torch.Tensor([-0.0266,  0.3486, -0.0049]).unsqueeze(0)
        self.all_points_std = torch.Tensor([0.3926])
    ...
    def __getitem__(self, item):
        ...
        point_cloud = (point_cloud - self.all_points_mean) / self.all_points_std
        ...
        # pc_utils.save_pointcloud(point_cloud.unsqueeze(0).numpy(),
        #                          save_path="./tmp.ply",
...
def build(args):
    train_dataset = ShapeNet15kPointClouds(
        ...
        root="./data/ShapeNetCore.v2.PC15k/",  # args.shapenet_data_dir,
